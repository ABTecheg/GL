...
{
    public partial class DepartmentCode : Form
    {
        ...
        private void DepartmentCode_Load(object sender, EventArgs e)
        {
            ...
            adapter = new SqlDataAdapter("Select * from DepartmentCode", sqlcon);
            cmdBuilder = new SqlCommandBuilder(adapter);
            adapter.Fill(dbAccountingProjectDS.DepartmentCode);
            dgv.DataSource = dbAccountingProjectDS.DepartmentCode;
        ...
        {
            adapter.Update(dbAccountingProjectDS.DepartmentCode);
            dbAccountingProjectDS.DepartmentCode.AcceptChanges();
        }
        ...
        private void btnSavenew_Click(object sender, EventArgs e)
        {
            ...
            if (GeneralFunctions.ValidateString(txtCode.Text, "Please Specify Code")
                && GeneralFunctions.ValidateString(txtDescription.Text, "Please Specify Description"))
            {
                sqlcon.Open();
                SqlCommand cmdSelect = new SqlCommand("Select ID from DepartmentCode where ID = '" + txtID.Text + "'", sqlcon);
                SqlDataReader dr = cmdSelect.ExecuteReader();
                if (!dr.HasRows && !GeneralFunctions.FindRow("Code = '" + txtCode.Text + "'", dbAccountingProjectDS.DepartmentCode))
                {
                    DataRow r = dbAccountingProjectDS.DepartmentCode.NewRow();
                    string id = txtID.Text;
                    r["ID"] = int.Parse(txtID.Text);
                    r["Code"] = txtCode.Text;
                    r["Description"] = txtDescription.Text;
                    r["Description2"] = txtDescription2.Text;
                    if (chkactive.Checked == true)
                        r["Active"] = "A";
                    else
                        r["Active"] = "I";                   
                    dbAccountingProjectDS.DepartmentCode.Rows.Add(r);
                    dr.Close();
                    sqlcon.Close();
                    ClearAll();
                    sqlcon.Open();
                    foreach (string accnum in DepartmentAccounts.accnum)
                    {
                        SqlCommand cmd = new SqlCommand("insert into Departmentsccounts values(" + id + ",'" + accnum + "')", sqlcon);
                        cmd.ExecuteNonQuery();
                    }
                    sqlcon.Close();
                    ClearAll();
                    DepartmentAccounts.accnum.Clear();
                    txtDescription2.Text = "";
                }
                else
                {
                    if (DialogResult.OK == MessageBox.Show("The Code already exists \n Preceed if you want to edit", "Alert", MessageBoxButtons.OKCancel))
                    {
                        DataRow[] rArr = dbAccountingProjectDS.DepartmentCode.Select("Code = '" + txtCode.Text + "'");
                        //rArr[0]["Code"] = txtCode.Text;
                        rArr[0]["Description"] = txtDescription.Text;
                        if (chkactive.Checked == true)
                            rArr[0]["Active"] = "A";
                        else
                            rArr[0]["Active"] = "I";
        ...
        private void Btnsaveedit_Click(object sender, EventArgs e)
        {
            if (dgv.SelectedRows.Count > 0)
            {
               ...
               && GeneralFunctions.ValidateString(txtDescription.Text, "Please Specify Description"))
                {
                    DataRow[] rArr = dbAccountingProjectDS.DepartmentCode.Select("ID = '" + txtID.Text + "'");
                    rArr[0]["Code"] = txtCode.Text;
                    rArr[0]["Description"] = txtDescription.Text;
                    rArr[0]["Description2"] = txtDescription2.Text;
                    if (chkactive.Checked == true)
                        rArr[0]["Active"] = "A";
                    else
                        rArr[0]["Active"] = "I";
        ...
        private void btndelete_Click(object sender, EventArgs e)
        {
            ...
            if (dgv.SelectedRows.Count > 0)
            {
                dbAccountingProjectDS.DepartmentCode.Rows[dgv.SelectedRows[0].Index].Delete();
                ClearAll();
            }
            else
            {
        ...
        private void btn_New_Click(object sender, EventArgs e)
        {
            ...
            sqlconBatch.Open();
            SqlCommand getBatch = new SqlCommand("Select Max(ID)+1 From DepartmentCode", sqlconBatch);
        ...
        private void dgv_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv.SelectedRows.Count > 0)
            {
                DataRow r = dbAccountingProjectDS.DepartmentCode.Rows[dgv.SelectedRows[0].Index];
                txtID.Text = r["ID"].ToString();
                txtCode.Text = r["Code"].ToString();
                txtDescription.Text = r["Description"].ToString();
                txtDescription2.Text = r["Description2"].ToString();
        ...
        {
            DepartmentAccounts obj = new DepartmentAccounts();
            obj.Show();
        }
    }
}