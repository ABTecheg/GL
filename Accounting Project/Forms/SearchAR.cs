...
{
    public partial class SearchAR : Form
    {
        public SearchAR()
        {
            InitializeComponent();
        }
        public string selectARNumber;
        ...
        private void SearchAP_Load(object sender, EventArgs e)
        {
            ...
            if (ApplyCredits == "Credits")
            {
                if (NOSearch != "")
                {
                    adaptertbBatch = new SqlDataAdapter("Select * From Batch Where BatchStat = 'P' AND BatchNo IN " + NOSearch, sqlcon);
                }

                else
                    adaptertbBatch = new SqlDataAdapter("Select * From Batch Where BatchSRC = 'ARP'", sqlcon);
            }
            else if (ApplyCredits == "ARP")
            {
                adaptertbBatch = new SqlDataAdapter("Select * From Batch Where BatchSRC = 'ARP'", sqlcon);
            }
            else
                adaptertbBatch = new SqlDataAdapter("Select * From Batch Where BatchSRC = 'AR'", sqlcon);
        ...
        private void dgv_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dgv.SelectedRows.Count > 0)
            {
                ...
                selectARNumber = r.Cells[0].Value.ToString();
                this.Close();
            }
        }
