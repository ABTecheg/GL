            ...
            try
            {
                ...
                if (txtReport == "Credit")
                {
                    s2 = "Create View RPTPaymentHistory AS SELECT TOP 100 PERCENT  dbo.Batch.BatchNo, dbo.Batch.BatchSRC, dbo.Batch.BatchStat, dbo.ARtrans.TransNO, dbo.ARtrans.TRANSDATE AS DateInvPayment, dbo.ARtrans.CodeTran AS Tran_Code, " +
                      ...
                      " WHERE  (dbo.Batch.BatchSRC = N'AR') AND (dbo.ARtrans.AmountLC < 0) AND (dbo.Batch.BatchStat = 'p') AND (dbo.ARCustomerMaintainance.AccountCode <> '') AND dbo.ARtrans.TRANSDATE Between '" + string.Format(GeneralFunctions.Format_Date, dtp_from.Value.Date) + "' AND '" + string.Format(GeneralFunctions.Format_Date, dtp_to.Value.Date) + "' ";
                }
                else if (txtReport == "Payment")
                {
                    s2 = "Create View RPTPaymentHistory AS SELECT TOP 100 PERCENT  dbo.Batch.BatchNo, dbo.Batch.BatchSRC, dbo.Batch.BatchStat, dbo.ARtrans.TransNO, dbo.ARtrans.TRANSDATE AS DateInvPayment, dbo.ARtrans.CodeTran AS Tran_Code, " +
                      ...
                      "dbo.ARtrans.AccountCode = dbo.ARCustomerMaintainance.AccountCode LEFT OUTER JOIN " +
                      "dbo.ARCreditDetails ON dbo.ARtrans.TransNO = dbo.ARCreditDetails.InvIDPay" +
                      " WHERE  (dbo.Batch.BatchSRC = N'ARP') AND (dbo.Batch.BatchStat = 'p') AND (dbo.ARCustomerMaintainance.AccountCode <> '') AND dbo.ARtrans.TRANSDATE Between '" + string.Format(GeneralFunctions.Format_Date, dtp_from.Value.Date) + "' AND '" + string.Format(GeneralFunctions.Format_Date, dtp_to.Value.Date) + "' ";
                }
                else
                {
                    s2 = "Create View RPTPaymentHistory AS SELECT TOP 100 PERCENT  dbo.Batch.BatchNo, dbo.Batch.BatchSRC, dbo.Batch.BatchStat, dbo.ARtrans.TransNO, dbo.ARtrans.TRANSDATE AS DateInvPayment, dbo.ARtrans.CodeTran AS Tran_Code, " +
                      ...
                      "dbo.ARtrans.AccountCode = dbo.ARCustomerMaintainance.AccountCode LEFT OUTER JOIN " +
                      "dbo.ARCreditDetails ON dbo.ARtrans.TransNO = dbo.ARCreditDetails.InvIDPay" +
                      " WHERE  ((dbo.Batch.BatchSRC = N'ARP') OR (dbo.ARtrans.AmountLC < 0)) AND (dbo.Batch.BatchStat = 'p') AND (dbo.ARCustomerMaintainance.AccountCode <> '') AND dbo.ARtrans.TRANSDATE Between '" + string.Format(GeneralFunctions.Format_Date, dtp_from.Value.Date) + "' AND '" + string.Format(GeneralFunctions.Format_Date, dtp_to.Value.Date) + "' ";
                }
                if (ListSelect.Items.Count > 0)
                {
                    if (rdAccountcode.Checked == true)
