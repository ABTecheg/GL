...
{
    public partial class RPTPurchseJournal : Form
    {
        public RPTPurchseJournal()
        {
            InitializeComponent();
        }
        ...
        private void RPTPurchseJournal_Load(object sender, EventArgs e)
        {
            ...
            {
                RPTPurJor.decmial = short.Parse(dr["DecimalPointsNumber"].ToString());
            }
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string s2 = "";
                s2 = "Create View RPTAPBatch AS SELECT TOP 100 PERCENT dbo.Batch.BatchNo, dbo.Batch.JVNumber, dbo.Batch.BatchDate, dbo.Batch.BatchPRD, dbo.Batch.BatchDSC, dbo.Batch.BatchJNL, dbo.Batch.BatchStat, " +
                      ...
                      "dbo.Users ON dbo.Batch.UserID = dbo.Users.UserID INNER JOIN dbo.APVendors ON dbo.APTrans.VendorCode = dbo.APVendors.VendorCode INNER JOIN dbo.APVendorCategory ON dbo.APVendors.VendorCategory = dbo.APVendorCategory.CategoryCode where dbo.Batch.BatchSRC = 'AP' AND Batch.BatchStat = 'P' AND Batch.BatchDate Between '" + string.Format(GeneralFunctions.Format_Date, dtp_from.Value.Date) + "' AND '" + string.Format(GeneralFunctions.Format_Date, dtp_to.Value.Date) + "'";
                if (ListSelect.Items.Count > 0)
                {
                    if (rdvendorcode.Checked == true)
