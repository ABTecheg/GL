...
{
    public partial class RPTAgeOpenItem : Form
    {
        public RPTAgeOpenItem()
        {
            InitializeComponent();
        }
        ...
        private void RPTAgeOpenItem_Load(object sender, EventArgs e)
        {
            ...
            {
                RPTAge_O_I_Details.decmial = short.Parse(dr["DecimalPointsNumber"].ToString());
                RPTAge_O_I_Summary.decmial = short.Parse(dr["DecimalPointsNumber"].ToString());
            }
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            try
            {
                if (rdsortcode.Checked == false && rdsortname.Checked == false && rdsortcategory.Checked == false && rdsortDueDate.Checked == false && rdsortinvdate.Checked == false)
                {
                ...
                commandReport2.CommandText = "if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[BudgetPlanning]') and OBJECTPROPERTY(id, N'IsView') = 1) Drop View RPTAgeOpenItem";
                try
                {
                ...
                { }
                string s2 = "";
                s2 = "Create View RPTAgeOpenItem AS SELECT TOP 100 PERCENT   dbo.Batch.BatchNo, dbo.APTrans.BatchInvoiceID, dbo.APTrans.VendorCode, dbo.APTrans.VendorName, dbo.APTrans.InvoiceDate, "+
                      "dbo.APTrans.TransactionType, dbo.APTrans.PayDate, dbo.APTrans.DueDays, dbo.APTrans.InvoiceAmount - dbo.APTrans.TaxValue AS Inv_Amount, "+
                      "dbo.APTrans.AmountPaid, dbo.APVendorCategory.CategoryDescription, dbo.Batch.BatchDate, "+
                      ...
                      "dbo.APGeneralSetup.ThirdPeriodDescription, dbo.APGeneralSetup.FourthPeriodDescription, dbo.APTrans.Reference FROM dbo.Batch INNER JOIN "+
                      "dbo.APTrans ON dbo.Batch.BatchNo = dbo.APTrans.APBatchNumber INNER JOIN "+
                      "dbo.APVendors ON dbo.APTrans.VendorCode = dbo.APVendors.VendorCode INNER JOIN "+
                      "dbo.APVendorCategory ON dbo.APVendors.VendorCategory = dbo.APVendorCategory.CategoryCode CROSS JOIN "+
                      "dbo.APGeneralSetup "+
                ...
                if (rdsortinvdate.Checked == true)
                    s2 = s2 + " ORDER BY dbo.APTrans.InvoiceDate";
                ...
                {
                    RPTAge_O_I_Details.DateAge = dtp_from.Value.Date;
                    RPTAge_O_I_Details rpt = new RPTAge_O_I_Details();
                    rpt.ShowDialog();
                }
                else
                {
                    RPTAge_O_I_Summary.DateAge = dtp_from.Value.Date;
                    RPTAge_O_I_Summary rpt = new RPTAge_O_I_Summary();
                    rpt.ShowDialog();
                }
