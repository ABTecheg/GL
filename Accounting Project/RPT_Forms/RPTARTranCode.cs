...
{
    public partial class RPTARTranCode : Form
    {
        public RPTARTranCode()
        {
            InitializeComponent();
        }
        ...
        private void rdDesc_CheckedChanged(object sender, EventArgs e)
        {
            if (rdDesc.Checked == true)
            {
                ...
                if (DTCust.Rows.Count != 0)
                {
                    ...
                    foreach (DataRow dr in DTCust.Rows)
                    {
                        if (!ListAvailable.Items.Contains(dr["TransDesc"]))
                            ListAvailable.Items.Add(dr["TransDesc"]);
                    }
                    ListAvailable.Sorted = true;
                }
            }
            ...
            try
            {
                ...
                if (ListSelect.Items.Count > 0)
                {
                    if (rdcode.Checked == true)
                        ...
                        s2 = s2 + " AND dbo.ARTransactionCodes.TransDesc IN ('";
                    string s = "";
                ...
                if (rdcode.Checked == true)
                    s2 = s2 + " ORDER BY dbo.ARtrans.CodeTran";
                else if (rdDesc.Checked == true)
                    s2 = s2 + " ORDER BY dbo.ARTransactionCodes.TransDesc";
                ...
                { }
                    RptARByTranCode rpt = new RptARByTranCode();
                    rpt.ShowDialog();
            }
            catch (Exception ex)
            {
